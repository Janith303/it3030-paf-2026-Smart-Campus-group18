...
export default function AdminTickets() {
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      // Member 4 fix: use axiosInstance so JWT token is attached
      await api.put(`/api/tickets/${selectedTicket.id}/status`, { status: newStatus, note: note });
                      ...
                      <tr key={ticket.id} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex justify-end gap-2">
                            ...
                            <button className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Assign Technician"
                              onClick={() => { setSelectedTicket(ticket); setShowModal(true); }}>
                              <UserPlus size={18} />
                            </button>
                            <button className="p-2 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors" title="Update Status"
                              onClick={() => { setSelectedTicket(ticket); setNewStatus(""); setNote(""); setShowUpdateModal(true); }}>
                              <Edit size={18} />
                            </button>
                          </div>
                        </td>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4"
          ...
          <div className="bg-white w-full max-w-md rounded-2xl shadow-xl p-8 animate-in fade-in zoom-in duration-200">
            ...
            <textarea value={note} onChange={(e) => setNote(e.target.value)}
              ...
              placeholder="Add details about this update..." rows={3} />
            <div className="flex gap-3">
